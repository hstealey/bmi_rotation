# -*- coding: utf-8 -*-
"""
Created on Tue Jan 16 17:20:44 2024
...
Manuscript: Neural population variance explains adaptation differences during learning 
...
PRE-REQS:
        Factor Analysis Results; Behavior (Distance, Time)
...
from stats_fxns            import sigSlope
...
for S in subject_list:
    ...
    print(np.mean(d50), np.mean(d90), np.mean(d50)-np.mean(d90))
...
Figure 3B - Average Behavior Over Sessions
______________________________________________________________________________
...
Figure 3C - BEHAVIOR - SCATTER PLOTS
______________________________________________________________________________
...
for S, axis, axis_sub in zip(subject_list, [ax[0], ax[1]], [ax2, ax3]):
    ...
    for i,k in enumerate(['bID', 'bIR', 'bMR']):
        ...
        print('t:{:.4f} ({:.3e} - {})'.format(t,p,equal_var))
...
Figure 4A - NEURAL PLOTS - Average %sv adaptation profile over sessions
______________________________________________________________________________
...
fig.text(0, 0.5, '% change in %sv'.format(chr(176)), va='center', fontsize=18, fontweight='bold', rotation=90)
...
for S, axis in zip(subject_list, [ax[0], ax[1]]):
        ...
        'Test if BL variances are within a ratio of < 4:1.'
        var1, var2 = np.var(mBL[ind50], ddof=1), np.var(mBL[ind90], ddof=1)
        
        # print(var1/var2)
        # print(var2/var1)
        
        t,p = stats.ttest_ind(mBL[ind50], mBL[ind90])
        print('Baseline Stats: t={:.3f} {:.3f}'.format(t,p))        
...
ax[0].set_xlim([-2,42])
ax[0].set_xticks([0, 5, 10, 15, 20, 25, 30, 35, 40])
...
ax[0].set_ylim([-18, 0])
ax[0].set_yticks([ -15, -10, -5, 0])
ax[0].set_yticklabels([ -15, -10, -5, 'BL'], fontsize=12)

ax[1].set_ylim([-18, 0])
ax[1].set_yticks([ -15, -10, -5, 0])
ax[1].set_yticklabels([ -15, -10, -5, 'BL'], fontsize=12)
...
# ax[0].set_ylim([0.33, 0.46])
# ax[0].set_yticks([0.34, 0.36, 0.38, 0.40, 0.42, 0.44, 0.46])
# ax[0].set_yticklabels([34, 36, 38, 40, 42, 44, 46], fontsize=12)

# ax[1].set_ylim([0.47,0.60])
# ax[1].set_yticks([0.48, 0.50, 0.52, 0.54, 0.56, 0.58, 0.60])
# ax[1].set_yticklabels([48, 50, 52, 54, 56, 58, 60], fontsize=12)
...
Figure 4B - NEURAL BAR PLOT -  Population Shared Variance
______________________________________________________________________________
...
#fig.text(0.5, 0.0, 'Behavioral Timepoint', ha='center', fontweight='bold',fontsize=14)
fig.text(0.0, 0.5, '% change in %sv', va='center', rotation=90, fontweight='bold', fontsize=20)
...
ax[0].set_yticks([-20, -15, -10, -5, 0])
ax[0].set_yticklabels([-20, -15, -10, -5, 'BL'], fontsize=16)
...
for S, axis in zip(subject_list, [ax[0], ax[1]]):
    ...
    for i,k in enumerate(['sID', 'sIR', 'sMR']): #['mBL', 'sID', 'sIR', 'sMR']
        ...
        r, p = stats.ttest_ind(r1, r2, equal_var=equal_var)
        print('{}, var:{} - {:.3f} ({:.1e})'.format(S, equal_var, r, p))
        print('\t{}'.format(len(r1)+len(r2)-2))
        
        if k == 'sMR':
            r, p = stats.ttest_1samp(r1, alternative='two-sided', popmean=0)
            print('\t50vBL(0) (n={}): {:.3f} ({:.1e})'.format(len(r1), r,p))
            r, p = stats.ttest_1samp(r2, alternative='two-sided', popmean=0)
            print('\t90vBL(0) (n={}): {:.3f} ({:.1e})'.format(len(r2), r,p))
        
     
        
    # print(S, '50 - BLvMA', stats.ttest_ind(dComp[S]['mBL'][ind50], dComp[S]['sMR'][ind50]))
    # print(S, '90 - BLvMA', stats.ttest_ind(dComp[S]['mBL'][ind90], dComp[S]['sMR'][ind90]))
    ...
    for m, s, degKey, x_axis in zip([m50, m90], [s50, s90], ['50', '90'], [[0,2,4], [0.75,2.75,4.75]]):
...
for p in Asig_values:
    ...
    Asig_stars.append(rstar)
    


ax[0].text(0,lineHeight*1.02,Asig_stars[0], fontsize=20)
ax[0].text(2,lineHeight*1.02,Asig_stars[1], fontsize=20)
ax[0].text(4,lineHeight*1.02,Asig_stars[2], fontsize=20, fontstyle='italic')
...
Bsig_values = [7.6e-02, 1.4e-02, 3.2e-04]
Bsig_stars = []
for p in Bsig_values:
    ...
    Bsig_stars.append(rstar)
    


ax[1].text(0.2,lineHeight*1.06,Bsig_stars[0], fontsize=20, fontstyle='italic')
ax[1].text(2.25,lineHeight*0.99,Bsig_stars[1], fontsize=20)
ax[1].text(4.,lineHeight*0.99,Bsig_stars[2], fontsize=20)
...
Figure 5A - Relationship: Behavior + Neural (SHARED)
______________________________________________________________________________
...
for S, axis in zip(subject_list, [ax[0], ax[1]]):
    ...
    for ind in dComp[S]['s_ind']:
        ...
        var1 = vSV#np.subtract(vSV, mBL[ind])/mBL[ind]
        var2 = vBE
...
ax[0].set_xticklabels([ -40, -20,  'BL', 20], fontsize=14)
ax[1].set_xticklabels([ -40, -20,  'BL', 20], fontsize=14)
...
Figure 5B - Relationship: Behavior + Neural (SHARED) - LINE FIT AT KEY POINTS
______________________________________________________________________________
...
for subject_ind, S in enumerate(subject_list):  
    ...
    for i, indDeg, marker, ls in zip([0], [indDeg], ['o'], ['--']):    
        ...
        for j, v1, v2, c, marker in zip([0,1,2], [sID, sIR, sMR],[bID, bIR, bMR], [c1,c2,c3], [marker1, marker2, marker3]):
            ...
            v1 = v1[indDeg]#np.subtract(v1[indDeg],mBL[indDeg])/mBL[indDeg] 
            v2 = v2[indDeg]
            ...
            if (slope_p < 1e-01) and (slope_p > 1e-02):
                slope_pstar = '*'
            elif (slope_p < 1e-02) and (slope_p > 1e-03):
                slope_pstar = '**'
            elif slope_p < 1e-03:
                slope_pstar = '***'
            else:
                slope_pstar = 'CHECK'
                print('{:.6f}'.format(slope_p))
            
            ax[j][subject_ind].text(-35,-2.75, 'slope: {:.3f} ({})'.format(slope,slope_pstar), fontweight='bold', fontsize=13)
...
ax[0][0].set_yticklabels(ylabs, fontsize=13)
ax[1][0].set_yticklabels(ylabs, fontsize=13)
ax[2][0].set_yticklabels(ylabs, fontsize=13)

'%change in %sv'
ax[2][0].set_xlim([-40,15])
ax[2][0].set_xticks([-40,  -20, -30,  -10, 0,  10])
ax[2][0].set_xticklabels([-40,  -30, -20, -10, 'BL',10], fontsize=13)
ax[2][1].set_xticklabels([-40,  -30, -20, -10, 'BL',10], fontsize=13)
...
Figure 5C - Relationship: Behavior + Neural (SHARED) - FIT OVER ALL POINTS
______________________________________________________________________________
...
for S, axis in zip(subject_list, [ax[0], ax[1]]):
    ...
    r = np.sqrt(num/den)
    print(r)
    #axis.text(25, 0.06, 'r\u00b2: {:.2f}  ({:.2f}e(-{:.2f}*x)+{:.2f})'.format(r, a,b,c))
    print('r\u00b2: {:.2f}  ({:.2f}e(-{:.2f}*x)+{:.2f})'.format(r, a,b,c))
...
# FIGURE 6: PREDICTION (LINEAR REGRESSION) - IR - TRIAL SET NUMBER OF MAX ADAPTATION

# Note: None (of BL, ID, IR, MR) are predictive of TSN
# ______________________________________________________________________________
...
# fig.text(0.55, 0, '%sv at IA Timepoint', fontweight='bold', ha='center', fontsize=16)
# fig.text(0,0.45, 'TSN at MA Timepoint', fontweight='bold', va='center', rotation=90, fontsize=16)
...
# # ax[0].set_title('Monkey A', fontsize=16)
# # #ax[0].set_xlim([0.27, 0.47])
# # ax[0].set_xticks(np.arange(0.28,0.48,0.02))
# # ax[0].set_xticklabels(['{:.0f}'.format(i*100) for i in np.arange(0.28,0.48,0.02)], fontsize=12)
# # ax[0].set_ylim([-1,42])
# # ax[0].set_yticks(np.arange(0,41,5))
# # ax[0].set_yticklabels(np.arange(0,41,5), fontsize=12)
# # #ax[0].set_xlabel('%sv at IA Timepoint', fontweight='bold', fontsize=10)


# # ax[1].set_title('Monkey B', fontsize=16)
# # ax[1].set_xticks(np.arange(0.46,0.65,0.02))
# # ax[1].set_xticklabels(['{:.0f}'.format(i*100) for i in np.arange(0.46,0.65,0.02)], fontsize=12)
...
FIGURE 6: PREDICTION (LINEAR REGRESSION) - ID, IR, MR - MAX ADAPTATION
...
fig, ax = plt.subplots(ncols=2, nrows=3, sharex=False, sharey=True, figsize=((5,12)))
...
for S, axis_list in zip(subject_list, [axA, axB]):
    ...
    for k, axis, c in zip(['sID', 'sIR', 'sMR'], axis_list, [magenta, yellow, blue]):
        ...
        axis.spines[['right', 'top']].set_visible(False)
        axis.set_xlim([-32,12])
        axis.set_xticks([-30, -20, -10, 0, 10])
        axis.set_xticklabels([ -30, -20, -10, 'BL', 10], fontsize=12)
        
        if S == 'Subject A':
            axis.set_ylabel('Max Adaptation', fontweight='bold', fontsize=14)
            axis.set_ylim([-0.5,0.3])
            axis.set_yticks([-0.4, -0.2, 0, 0.2])
            axis.set_yticklabels([-0.4, -0.2, 'BL', 0.2], fontsize=12)
        ...
        if S == 'Subject A':
            axis.text(-20,0.1, 'r: {:.2f} ({})'.format(r,rstar), fontsize=14, fontweight='bold')
        else:
            axis.text(-20,0.1, 'r: {:.2f} ({})'.format(r,rstar), fontsize=14, fontweight='bold')
            #print(p, rstar)
    ...
    mBL = dComp[S]['mBL'][ind]
    adaptation = dComp[S]['bMR'][ind]
        
    print('mBL', stats.pearsonr(mBL, adaptation))


ax[0][0].set_xlabel('%change in %sv at ID Timepoint', fontweight='bold', fontsize=14)
#ax[0][1].set_xlabel('%change in %sv at ID Timepoint', fontweight='bold', fontsize=12)

ax[1][0].set_xlabel('%change in %sv at IA Timepoint', fontweight='bold', fontsize=14)
#ax[1][1].set_xlabel('%sv at IA Timepoint', fontweight='bold', fontsize=12)

ax[2][0].set_xlabel('%change in %sv at MA Timepoint', fontweight='bold', fontsize=14)
...
#     v1 = []
#     for k in range(40):
#         v1.append(np.mean(dLDA_eachTSN[S]['model'][k,:,:]))
    
#     print(np.min(v1), np.max(v1), np.max(v1)-np.min(v1))
